(async ()=>{
    ...
    function $() {
        return R || (R = (async ()=>{
            const [t, o] = await Promise.all([
                import("./pdf-B9xD6m9y.js").then(async (m)=>{
                    await m.__tla;
                    return m;
                }),
                import("./pdf.worker.min-CkLT2Pwd.js").then((e)=>e.default)
            ]);
            return t.GlobalWorkerOptions.workerSrc = o, t;
        })()), R;
    }
    async function W() {
        return $();
    ...
    function q(t) {
        ...
        for (const e of t.items){
            ...
            const s = Math.abs(e.transform[0]) || Math.abs(e.transform[3]), n = e.fontName || "", a = F(n, e.transform);
            o.push({
                str: e.str.replace(/[\r\n]+/g, " "),
                x: e.transform[4],
                y: e.transform[5],
    ...
    function Q(t, o, e) {
        ...
        if (o) {
            ...
            return u.length > 0 && c.push(u), c.map((i)=>k(i, e));
        }
        const s = [];
        let n = [
            t[0]
        ...
        return n.length > 0 && s.push(k(n, e)), s;
    }
    function D(t) {
        const o = [];
        let e = null;
    ...
    function G(t, o) {
        ...
        for(let r = 1; r < t.length; r++)t[r].x - (t[r - 1].x + t[r - 1].width) > t[r].fontSize * 2 ? (n.push(D(a)), a = [
            t[r]
        ]) : a.push(t[r]);
        a.length > 0 && n.push(D(a));
    ...
    function Z(t, o) {
        ...
        for(let n = 1; n < t.length; n++){
            ...
            Math.abs(a.y - c) > u ? (e.push(G(s, o)), s = [
                a
            ]) : s.push(a);
        }
        return s.length > 0 && e.push(G(s, o)), e;
    }
    function K(t) {
    ...
    function ot(t, o) {
        ...
        for(; n < t.length;){
            ...
            const c = X.exec(a.text), u = H.exec(a.text);
            if (!c && !u) {
                n++;
                continue;
            }
            const i = c ? "bullet" : "numbered", f = c ? X : H, l = a.xStart, r = [];
            for(; n < t.length;){
                const h = t[n], p = f.exec(h.text);
                if (p && Math.abs(h.xStart - l) < h.fontSize * 3) {
                    const m = nt(h.segments, p[0].length), g = h.text.slice(p[0].length), d = Math.max(0, Math.round((h.xStart - l) / (h.fontSize * 2)));
                    r.push({
                        segments: m,
                        text: g,
                        level: d
                    ...
                    m.text += " " + h.text, st(m.segments, h.segments), e.add(n), n++;
                } else break;
            }
            r.length >= 1 ? s.push({
                type: "list",
    ...
    const it = 0, ct = 1, lt = 2, ft = 3, ht = 4;
    function ut(t, o, e, s, n) {
        let a = 0, c = 0, u = 0, i = 0, f = 0;
        for(; f < t.length;){
            const l = t[f++] | 0;
            if (l === it) a = t[f++], c = t[f++], u = a, i = c;
            else if (l === ct) {
                const r = t[f++], h = t[f++], [p, m] = E(o, a, c), [g, d] = E(o, r, h);
                _(p, m, g, d, e, s, n), a = r, c = h;
            } else if (l === lt) a = t[f + 4], c = t[f + 5], f += 6;
            else if (l === ft) a = t[f + 2], c = t[f + 3], f += 4;
            else if (l === ht) {
                if (a !== u || c !== i) {
                    const [r, h] = E(o, a, c), [p, m] = E(o, u, i);
                    _(r, h, p, m, e, s, n);
                }
                a = u, c = i;
            } else break;
    ...
    function gt(t, o) {
        ...
        for(let u = 0; u < t.fnArray.length; u++){
            ...
            if (i === o.OPS.transform) {
                a = rt(a, f[0], f[1], f[2], f[3], f[4], f[5]);
                continue;
            }
            if (i === o.OPS.save) {
                c.push([
            ...
            if (i === o.OPS.constructPath) {
                ...
                r && typeof r.length == "number" && r.length > 0 && ut(r, a, 1.5, e, s);
            }
        }
        return {
            horizontal: e,
    ...
    function dt(t, o, e, s) {
        const c = t.filter((d)=>d.y1 > 2 && d.y1 < s - 2), u = o.filter((d)=>d.x1 > 2 && d.x1 < e - 2), i = N(c.map((d)=>d.y1), 3), f = N(u.map((d)=>d.x1), 3);
    ...
    function pt(t, o, e, s, n) {
        const a = new Set, c = [], { horizontal: u, vertical: i } = gt(t, o), f = dt(u, i, s, n);
        ...
        for (const l of f){
            ...
            for(let C = 0; C < e.length; C++){
                ...
                let w = -1;
                for(let y = 0; y < r; y++){
                    const v = l.rowYs[y], P = l.rowYs[y + 1];
                    if (I.y <= v + S && I.y >= P - S) {
                        w = y;
                        break;
                    }
                ...
                let M = -1;
                for(let y = 0; y < h; y++){
                    const v = l.colXs[y], P = l.colXs[y + 1];
                    if (I.x >= v - S && I.x < P + S) {
                        M = y;
                        break;
                    }
    ...
    async function bt(t) {
        const o = await yt(t);
        if (!o) return [];
        const { createWorker: e } = await import("./index-DIi5oGTW.js").then(function(a) {
            return a.i;
        }), s = self.location.origin, n = await e("vie+eng", void 0, {
            workerPath: `${s}/libs/tesseract/worker.min.js`,
            corePath: `${s}/libs/tesseract/core`,
            langPath: `${s}/models/tessdata`
        });
        try {
            const { data: a } = await n.recognize(o);
            return !a.text || !a.text.trim() ? [] : a.text.split(/\n\s*\n/).map((u)=>u.replace(/\n/g, " ").trim()).filter(Boolean).map((u, i)=>({
                    type: "paragraph",
                    segments: [
                        {
                            text: u,
    ...
    async function St(t, o) {
        ...
        for(let w = 0; w < t.length; w++){
            ...
            if (!o || T.length === 0) for (const y of M)Y(y) && x.push(...C(y));
            else {
                const y = [];
                for (const v of M)Y(v) && y.push({
                    kind: "block",
                    y: -v.y,
                    block: v
                });
    ...
    function It(t, o, e, s) {
        const n = et(t, o), a = ot(n, s), c = n.filter((l, r)=>!a.consumed.has(r)), u = at(c, o, s), i = c.filter((l, r)=>!u.consumed.has(r));
        return [
            ...Q(i, e, s),
            ...a.blocks,
            ...u.blocks
    ...
    async function Mt(t, o, e) {
        e(2, "Đang tải thư viện...");
        const s = await W();
        e(5, "Đang đọc file PDF...");
        const n = await V(s, t), a = n.numPages, c = [], u = [];
        for(let r = 1; r <= a; r++){
            const h = 10 + Math.round(r / a * 30);
            e(h, `Đang trích xuất trang ${r}/${a}...`);
            const p = await n.getPage(r), m = p.getViewport({
                scale: 1
            }), g = m.width, d = await p.getTextContent(), x = q(d), S = await p.getOperatorList(), { tables: b, consumedIndices: C } = pt(S, s, x, g, m.height), I = C.size > 0 ? x.filter((M, T)=>!C.has(T)) : x;
            let w = Z(I, g);
            w = K(w), c.push({
                lines: w,
                pageWidth: g,
        ...
        for(let r = 0; r < a; r++){
            ...
            let x;
            p.length === 0 && g.length === 0 && o.enableOCR ? (e(h, `Đang OCR trang ${r + 1}/${a}...`), x = await bt(d)) : x = [
                ...It(p, m, o.preserveLayout, i),
                ...g
            ].sort((C, I)=>I.y - C.y);
            const S = o.includeImages ? await wt(d, s) : [];
            f.push({
                blocks: x,
                images: S
            }), c[r] = {
        ...
        const l = await St(f, o.includeImages);
